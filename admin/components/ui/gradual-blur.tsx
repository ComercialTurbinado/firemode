...
function useResponsiveDimension(
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      const mobile = config[`mobile${cap}` as keyof GradualBlurConfig];
      const tablet = config[`tablet${cap}` as keyof GradualBlurConfig];
      const desktop = config[`desktop${cap}` as keyof GradualBlurConfig];
